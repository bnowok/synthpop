...
{ 
  ...
  allCAPs         <- matrix(NA,object$m,8)
  attrib   <- matrix(NA,object$m,9)
  ident  <- matrix(NA,object$m ,4)
  attrib_denoms  <- matrix(NA,object$m ,4)
  
  dimnames(allCAPs) <- list(1:object$m, c("baseCAPd","CAPd", "CAPs", "DCAPb","DCAPs","TCAPb","TCAPs","TCAP"))
  dimnames(attrib) <- list(1:object$m,c("Dorig","Dsyn","iS", "DiS","DiSCO", "DCAPd", "DiSDiO", "max_denom","mean_denom"))
  dimnames(ident) <- list(1:object$m,c("UiO", "UiS","UiOiS", "repU"))
  dimnames(attrib_denoms) <- list(1:object$m, c("denom_DiSCO","denom_TCAPb", "denom_TCAPs","denom_TCAP"))
  ...
  for ( jj in 1:object$m) {
    ...
    Nsboth <- sum(tab_ks[tab_kd >0])
    
    DCAPs <-         sum(apply(tab_kts_p*tab_ktd,2,sum))/Ns*100
    DCAPd  <-    sum(apply(tab_kts_p*tab_ktd,2,sum))/Nd*100
    DCAPb <-   sum(apply(tab_kts_p*tab_ktd,2,sum))/Nsboth*100
    
    attrib[jj,] <- c( Dorig,Dsyn,iS,DiS,DiSCO,DCAPd,DiSDiO,max(tab_DiSCO),mean(tab_DiSCO[tab_DiSCO>0]))
    Nexclusions[[jj]] <- Nexcludes
    
    ##cat(Ns,Nsboth,DCAP,DCAPalt, "Ns,NSboth,DCAP,DcAPalt\n")
    
    ##restrict to uniques in d
    
    tab_kts_pU <- tab_kts_p
    tab_kts_pU[tab_kts_p != 1] <- 0
    TCAPb <-   sum(tab_kts_pU*tab_ktd)/Nsboth*100
    TCAPs <-   sum(tab_kts_pU*tab_ktd)/Ns*100
    
    denom_TCAP <- sum(tab_kd * apply(tab_kts_pU, 2,sum))
    
    if (denom_TCAP == 0) TCAP <- NA else
      TCAP <-   sum(tab_kts_pU*tab_ktd)/denom_TCAP*100
    
    
    allCAPs[jj,] <- c( baseCAPd,  CAPd,  CAPs, DCAPb, DCAPs, TCAPb,TCAPs, TCAP)
    attrib_denoms[jj,] <- c( Nd,Ns, Nsboth, denom_TCAP)
  ...
  res <- list(ident = data.frame(ident), attrib = data.frame(attrib), 
              allCAPs = data.frame(allCAPs),
              attrib_denoms = data.frame(attrib_denoms),
              check_1way = check_1way, 
