...
The same two choices of denominator as were used for $DCAP$ could be used. One choice $TCAP_{s}$ would be to use the denominator, $N_{s}$ and the other $TCAP_{b}$ would be to use the denominator, $N_{b}$. Note that when $N_s = N_d$, the measure $TCAP_s$ will be identical to $DiSCO$.
However the $TCAP$ that is defined in \cite{little2022, little2025} does not use either of these denominators, but uses as a denominator $N_{bp}$ the number of GT records with $q$ present in the SD and where any level of the target $t$ is predicted with certainty. These are the records identified at the second bullet point of the procedure described in Section 3.3.1 above. The numerators and denominators used for each of these measures are summarised in Table 1.
