...
We would also like to thank Timon Huijser, for pointing out errros in a previous version of this vignette.
...
as $DISCO$, but uses as denominator the number of records in SD that have keys represented in the original data. It can also be scaled from $baseCAPD$ to 1,
